...
The grimoire now develops six schools at the user's direction: Enchantment (group theory), Transmutation (category theory and the algebraic foundations of topology), Illusion (complex analysis), Phantasms (affine algebraic geometry, linking Illusion and Transmutation), Divination (Augury/probability, Scrying/statistics, Arithmancy/number theory), and Necromancy (real analysis and introductory functional analysis, linking back to Enchantment). Preserve the substantial Enchantment progression while giving each new school concrete computations and precise foundations. PDEs are outside the present scope.
...
- Spell numeric literals use digit-by-digit kanji (`0123456789` → `〇壱弐参四五六七八九`; the formal 壱 弐 参 avoid 一 二 三, which read as a dash, `=` and `≡`), preserving leading zeros, decimal places, base prefixes, and exponent spelling without numeric evaluation. Number lexing and editor highlighting share the same patterns. Reserve numeral-leading spell names so namespace components cannot absorb numeric projections; leave strings, comments, and quoted identifiers literal.
...
- Keep spell names short: at most four content words and 28 letters (`src/naming.ts`); `npm run grimoire:verify` rejects longer ones.
- Keep the translation key with exported drafts; it carries the information needed to reverse renamed identifiers.
- Keep actual Lean namespaces, declarations, and proof locals mathematical. The book lives in mathematically named `Mathematics.*` namespaces; `grimoire/lexicon.json` holds the spell-side names. Its optional `namespaces` map translates full prefixes, so the Enchantment alias does not rename `Mathlib.GroupTheory`.
...
- Mathematical sources: `math/Mathematics/`; lesson explanations and vocabulary: `grimoire/chapters.json` and `grimoire/lexicon.json`. `grimoire/schools.json` defines the school/subschool navigation; each folio must belong to one of its groups (except shared Cantrips). The verifier includes the school registry and its hash in the generated catalog.
