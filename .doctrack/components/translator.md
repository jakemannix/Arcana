...
Spell-side names are limited by `src/naming.ts`: at most four content words (small words such as "the" and "of" are free) and 28 letters. The verifier refuses a lexicon that breaks the limit, and a test checks the saved key. `[ ]` read `⁅ ⁆` and `×` reads `※`, because the older glyphs looked like parentheses and a multiplication sign.

## Machine interface (2026-09-24)
`scripts/arcana-cli.ts` exposes `lookup`, `names` and `translate` over stdin/stdout JSON for tool services (the Mathemagician's lean-service in `yetanotheruseless/anyalignment-deploy`). Each call builds a fresh `Key` from the verified grimoire key, so proposed names never leak into the book. Name checks combine key validation (uniqueness, reserved words, valid spell names), `nameLengthProblem`, and a declaration style rule (single capitalized word or sparkle-framed title). `translate` reports automatically allocated names with a `declaration`/`other` role so an agent can replace them. `vite.cli.config.ts` builds the standalone `dist-cli/arcana-cli.js` bundle (publicDir off). `tests/cli.test.ts` requires exact reproduction of every folio spell.
